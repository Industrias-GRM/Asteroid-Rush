...
// ============================================================
// FIRESTORE NOTICIAS (SER P/ Noticias)
// ============================================================
...
// Guardamos la hora del servidor + el instante de la lectura.
// El cliente calcula: horaServidor + (performance.now() − syncMono).
...
// syncServerTime: el popup envía su performance.now() para que
// el cálculo monótnico sea preciso. El SW guarda ese valor.
...
async function checkAndRefreshCaches() {
  ...
  for (const mode of modes) {
    ...
    // Regla 5 y 6: Si el caché tiene más de 20 min o no existe, descargar
...
/**
 * ============================================================
 * FIRESTORE NEWS: cooldown 24h (cache del SW) - aislado del ranking
 * Condicional requerido:
 * - Si han pasado MÁS DE 24h desde la ULTIMA SOLICITUD al servidor -> volver a solicitar
 * - Si no han pasado MÁS DE 24h -> devolver la lista guardada de la última solicitud
 * ============================================================
 */
const NEWS_COOLDOWN_MS = 24 * 60 * 60 * 1000;

/**
 * "última solicitud" (persistida)
 * - En SW no existe localStorage: usamos chrome.storage.local
 * - Así el SW puede apagarse/encenderse y seguir respetando el cooldown de 24h
 */
...
async function getNewsWithCooldown({ forceRefresh = false } = {}) {
  ...
  // 1) Tomar "última solicitud" desde persistencia (o fallback a cache en memoria)
  const persistedLastRequestAt = await _getPersistedNewsLastRequestAt();
  const lastRequestAt =
    persistedLastRequestAt ||
    (typeof newsCache.lastRequestAt === "number" ? newsCache.lastRequestAt : (newsCache.lastFetch || 0));

  const timeSinceLastRequest = now - (lastRequestAt || 0);

  // 2) Sólo devolvemos caché si:
  //    - existe data en memoria (la lista guardada en runtime del SW)
  //    - NO se ha pedido forzar refresco
  //    - NO han pasado más de 24h desde la última solicitud
  ...
  // 3) FETCH real (actualizamos "última solicitud" ANTES del request, persistido)
  ...
  // 4) fallback offline si existe cache en memoria
  if (newsCache.data) {
...
async function getNews() {
  ...
  try {
    ...
    // Firestore REST devuelve docId en name: projects/.../documents/Noticias/{docId}
    const items = documents.map(doc => {
      ...
      // Devolvemos todo el documento para que el cliente seleccione el idioma con Titulo_(idioma)/Noticia_(idioma)
      return { docId, date, fields };
    });

    // Orden por fecha (ascendente => antiguas primero). El cliente luego renderiza como necesita.
    items.sort((a, b) => (_parseNewsDateToMs(a.date) ?? -Infinity) - (_parseNewsDateToMs(b.date) ?? -Infinity));

    // Reducimos para que el cliente sólo necesite campos de título/text por idioma.
    // Mantenemos fields originales.
...
// ============================================================
// CACHE ESTÁTICO (#15) — precache de assets críticos
// ============================================================
